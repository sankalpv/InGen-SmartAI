...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/builder-productivity', label: 'Builder Productivity', icon: BarChart2 },
...
export default function Sidebar() {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--sidebar-width', isOpen ? '280px' : '72px');
  ...
  useEffect(() => {
    fetch('/api/settings/config')
      ...
      .then((data) => {
        ...
        if (s.deploymentMode !== 'hosted' && s.outlookIntegration === false)
  ...
  const filterItems = (items) => (outlookEnabled ? items : items.filter((i) => !i.requiresOutlook));
  ...
  return (
    <aside className="ingen-sidebar" data-pin={dataPinValue}>
      ...
      >
        ...
        {/* Divider */}
        <div
          style={{ height: '1px', background: 'rgba(255,255,255,0.06)', margin: '4px 2px 8px' }}
        />
            ...
            return (
              <div key={group.id}>
                ...
                >
                  ...
                  <span
                    className="ingen-sidebar-label"
                    style={{ flex: 1, textAlign: 'left', fontSize: '0.95rem' }}
                  >
                    {group.label}
                  </span>
                  <span
                    className="ingen-sidebar-label"
                    style={{ flexShrink: 0, opacity: 0.5, display: 'flex', alignItems: 'center' }}
                  >
                  ...
                  {!isExpanded && (
                    <span
                      ...
                      style={{
                        fontSize: '11px',
                        background: hasActive ? 'rgba(139,92,246,0.15)' : 'rgba(255,255,255,0.06)',
                ...
                <div
                  className={`ingen-section-body${isExpanded ? ' expanded' : ''}`}
      ...
      <div style={{ padding: '8px 10px 16px', flexShrink: 0 }}>
        ...
        >
          {isOpen ? (
            <PanelLeftClose size={16} style={{ flexShrink: 0 }} />
          ) : (
            <PanelLeftOpen size={16} style={{ flexShrink: 0 }} />
          )}
        ...
        >
          ...
          <span className="ingen-sidebar-label">
