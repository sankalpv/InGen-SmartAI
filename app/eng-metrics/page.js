...
const METRIC_DESCRIPTIONS = {
  crsCreated: {
    ...
    calculation:
      'Counted from code.amazon.com user activity via amzn-mcp search_internal_code (type=user). Each unique CR where the engineer is listed as the author is counted once.',
    benchmark:
      '5-8 CRs/week is a healthy range for an IC. Higher for senior engineers working on smaller, incremental changes.',
    icon: '📝',
  },
  crsReviewed: {
    title: 'CRs Reviewed',
    ...
    calculation:
      'Counted from code.amazon.com user activity. Each CR where the engineer appears as a reviewer (approved, commented, or requested changes) is counted.',
    benchmark:
      'A healthy review ratio is 1.5–2× the number of CRs created. This ensures knowledge sharing and reduces bus factor risk.',
    icon: '👀',
  },
  p50Turnaround: {
    ...
    calculation:
      'For each engineer, their average CR turnaround time is extracted from code.amazon.com activity. The P50 (median) is then computed across all engineers with data.',
    benchmark:
      'Under 4 hours is excellent. 4-8 hours is acceptable. Over 8 hours indicates review bottlenecks that slow development velocity.',
    icon: '⏱️',
  },
  staleCrs: {
    title: 'Stale CRs',
    description:
      'Open CRs that have been waiting for review for more than 5 days with no activity. Shown as an org-level alert only — not attributed to individual engineers since reviewers control turnaround.',
    calculation:
      'CRs in "open" status where the creation/last-activity date is >5 days ago. Configured via engMetrics.staleCrThresholdDays in settings.json. Aggregated across the entire org.',
    benchmark:
      'Target: 0 stale CRs. Each stale CR represents blocked work and potential context-switching cost when the engineer returns to it.',
    icon: '🔴',
  },
  reviewRatio: {
    title: 'Review Ratio',
    description:
      "The ratio of CRs reviewed to CRs created for each engineer. Measures engineering citizenship — how much an engineer invests in reviewing teammates' code relative to their own output.",
    calculation:
      'CRs Reviewed ÷ CRs Created for the current week. A ratio of 1.5× means the engineer reviewed 50% more CRs than they authored. Edge cases: if CRs Created = 0 but reviews > 0, shows ∞.',
    benchmark:
      '≥1.5× is healthy (reviewing more than authoring promotes knowledge sharing). 1.0–1.5× is acceptable. <1.0× means reviewing less than creating — consider encouraging more review activity.',
    icon: '🔄',
  },
};
    ...
    <span ref={ref} style={{ position: 'relative', display: 'inline-flex', ...style }}>
      <button
        ...
        style={{
          ...
          cursor: 'pointer',
          padding: '2px',
          color: open ? '#0a84ff' : 'rgba(255,255,255,0.2)',
        ...
        onMouseLeave={(e) => {
          if (!open) e.currentTarget.style.color = 'rgba(255,255,255,0.2)';
        }}
...
function TrendDelta({ delta, suffix = '' }) {
  if (delta === null || delta === undefined)
    return (
      <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.15)', marginLeft: '6px' }}>
        —
      </span>
    );
  if (delta === 0)
...
function Sparkline({
  ...
  highlightColor = 'rgba(48,209,88,0.6)',
}) {
...
function AlertCard({ icon, count, label, color, description, onClick }) {
  ...
  const countColor =
    color === 'red' ? '#ff453a' : color === 'orange' ? '#ff9f0a' : 'rgba(255,255,255,0.7)';
  ...
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontSize: '22px', fontWeight: 700, color: countColor, lineHeight: 1 }}>
          {count}
        </div>
        <div
          style={{
        ...
        >
          <span
            style={{
              width: '48px',
              fontSize: '11px',
              color: i === trend.length - 1 ? '#0a84ff' : 'rgba(255,255,255,0.35)',
              textAlign: 'right',
              flexShrink: 0,
              fontWeight: i === trend.length - 1 ? 700 : 400,
            }}
          >
            {t.weekLabel}
          </span>
        ...
        {loading && (
          ...
          >
            ...
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)', marginTop: '4px' }}>
              Fetching 12-week history from eng-metrics.db
            </div>
          </div>
        )}
          ...
          <>
            ...
            {detail.currentWeek && (
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '24px' }}>
                <OrgStat
                  value={detail.currentWeek.crsCreated}
                  label="CRs Created"
                  bgClass="blue"
                  metricKey="crsCreated"
                />
                <OrgStat
                  value={detail.currentWeek.crsReviewed}
                  label="Reviewed"
                  bgClass="green"
                  metricKey="crsReviewed"
                />
              </div>
            )}
            ...
            <div
              style={{
                background: 'linear-gradient(145deg, rgba(10,132,255,0.08), rgba(139,92,246,0.05))',
            ...
            >
              <div
                style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'space-between',
                  marginBottom: summaryText || summaryLoading ? '12px' : '0',
                }}
              >
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch(
                          `/api/eng-metrics?view=work-summary&alias=${alias}`
                        );
                        if (!res.ok) {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>
                    {summaryPhase === 'fetching'
                      ? 'Fetching CR details from code.amazon.com...'
              ...
              {crDetails && crDetails.length > 0 && (
                <div
                  style={{
                    ...
                    gap: '4px',
                    flexWrap: 'wrap',
                    marginBottom: summaryText ? '10px' : '0',
                  }}
                >
                  {crDetails.map((cr, i) => (
                    <a
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        background:
                          cr.type === 'created' ? 'rgba(10,132,255,0.1)' : 'rgba(48,209,88,0.1)',
                        color: cr.type === 'created' ? '#0a84ff' : '#30d158',
                        border: `1px solid ${cr.type === 'created' ? 'rgba(10,132,255,0.15)' : 'rgba(48,209,88,0.15)'}`,
                        textDecoration: 'none',
                      }}
                    >
              ...
              {!summaryLoading && !summaryText && (
                <div
                  style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', marginTop: '8px' }}
                >
                  Click &quot;Generate Summary&quot; to fetch live CR details and create an
                  ...
                  return (
                    <div key={h.weekId}>
                      <div
                        ...
                        onMouseEnter={(e) => {
                          if (hasCrs) e.currentTarget.style.background = 'rgba(10,132,255,0.06)';
                        }}
                        onMouseLeave={(e) => {
                          if (!isExpanded)
                      ...
                      >
                        ...
                        <span
                          style={{
                            width: '40px',
                            color: isCurrent ? '#0a84ff' : 'rgba(255,255,255,0.35)',
                            flexShrink: 0,
...
function YearTrendChart({ data, metric = 'crsCreated' }) {
  ...
  if (!data || data.length === 0) return null;
  const metricLabels = {
    crsCreated: 'CRs Created',
    crsReviewed: 'CRs Reviewed',
  ...
  const padding = { top: 30, right: 50, bottom: 30, left: 40 }; // Increased right padding to prevent last week tooltip from clipping
    ...
    <div style={{ overflowX: 'auto', marginBottom: '8px' }}>
      <svg width={chartWidth} height={chartHeight} style={{ display: 'block' }}>
        ...
        {values.map((v, i) => {
          ...
          const x = padding.left + (i / Math.max(values.length - 1, 1)) * innerW;
          const y = padding.top + innerH - (v / maxVal) * innerH;
          const isHovered = hover?.week === data[i].weekLabel;
          return (
            <g
              key={i}
              onMouseEnter={() => setHover({ v, week: data[i].weekLabel, x, y })}
        ...
        {hover && (
          <g pointerEvents="none">
            ...
            >
              {hover.v}{' '}
              <tspan fill="rgba(255,255,255,0.5)" fontWeight="normal">
                {hover.week}
              </tspan>
            </text>
          </g>
...
export default function EngMetricsPage() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      fetch('/api/eng-metrics?view=stale-crs')
        .then((r) => r.json())
        .then((d) => {
            ...
            setDashboard((prev) =>
              prev
                ? {
                    ...
                    summary: {
                      ...prev.summary,
                      staleCrs: { value: d.data.count, prev: prev.summary?.staleCrs?.prev || 0 },
                    },
                    alerts: {
                      ...prev.alerts,
                      staleCrs: d.data.count,
                      staleCrDetails: d.data.details || [],
                    },
                  }
                : prev
            );
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setRefreshProgress(
          `Fetched ${data.data?.fetchedCount || 0} engineers. Reloading dashboard...`
        );
  ...
  const handleCancelBackfill = async () => {
    try {
      await fetch('/api/eng-metrics?view=backfill-cancel');
    } catch (e) {
      /* ignore */
    }
  };
    ...
    <div className="dark-inline-page" style={{ zoom: 1.15 }}>
        ...
        <div>
          ...
          >
            {dashboard && (
              <>
                <span>
                  {dashboard.weekId} · {dashboard.totalEngineers} engineers
                </span>
                <button
                  onClick={handleRefresh}
                  disabled={isRefreshing || isBackfilling}
                  style={{
                    background: isRefreshing ? 'rgba(255,255,255,0.05)' : 'rgba(139,92,246,0.15)',
                    color: isRefreshing ? 'rgba(255,255,255,0.3)' : '#a78bfa',
                    border: 'none',
                    padding: '6px 16px',
                    borderRadius: '8px',
                    fontSize: '13px',
                ...
                {missingWeeks > 0 && (
                  <button
                    ...
                    style={{
                      ...
                      color: isBackfilling ? 'rgba(255,255,255,0.3)' : '#0a84ff',
                      border: 'none',
                      padding: '6px 16px',
                      borderRadius: '8px',
                      fontSize: '13px',
                  ...
                  >
                    ...
                    {isBackfilling
                      ? 'Backfilling...'
                      : `📥 Fetch ${currentYear} Data (${missingWeeks} weeks)`}
                  </button>
                )}
                <div
                  style={{
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      cursor:
                        slackStatus === 'sending' || !slackChannel.trim()
                          ? 'not-allowed'
                          : 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                ...
                {slackStatus === 'error' && slackError && (
                  <span style={{ fontSize: '11px', color: '#ff453a' }} title={slackError}>
        ...
        >
          ...
          <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '12px' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.3)', marginLeft: '8px' }}>
              · {backfillProgress.totalEngineers} engineers · Phase:{' '}
              {backfillProgress.currentPhase || 'starting'}
            </span>
          </div>
      ...
      {error && (
        ...
        >
          ...
          <div style={{ color: '#ff453a', fontWeight: 700, marginBottom: '6px', fontSize: '15px' }}>
      ...
      {!isLoading && !error && dashboard?.empty && (
        ...
        >
          ...
          >
            Click &quot;Refresh&quot; to fetch code review metrics from code.amazon.com for all
            engineers in your org. Make sure your org is synced first (Settings → Org Sync).
          </p>
          <button
            onClick={handleRefresh}
          ...
          >
            Uses amzn-mcp → search_internal_code (type=user) to query per-engineer CR activity.
            <br />
      ...
      {!isLoading && !error && dashboard && !dashboard.empty && (
        <>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '24px' }}>
            <OrgStat
              value={dashboard.summary.crsCreated.value}
              label="CRs Created"
              trend={dashboard.summary.crsCreated.trend}
              trendLabel={`${Math.abs(dashboard.summary.crsCreated.trend)}% vs last week`}
              bgClass="blue"
              metricKey="crsCreated"
            />
            <OrgStat
              value={dashboard.summary.crsReviewed.value}
              label="CRs Reviewed"
              trend={dashboard.summary.crsReviewed.trend}
              trendLabel={`${Math.abs(dashboard.summary.crsReviewed.trend)}%`}
              bgClass="green"
              metricKey="crsReviewed"
            />
            <OrgStat
              value={dashboard.summary.staleCrs.value}
              label="Stale CRs"
              trend={dashboard.summary.staleCrs.value <= dashboard.summary.staleCrs.prev ? 1 : -1}
              trendLabel={`was ${dashboard.summary.staleCrs.prev} last week`}
              bgClass="red"
              metricKey="staleCrs"
            />
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '28px' }}>
            <AlertCard
              icon="🔴"
              count={dashboard.alerts.staleCrs}
              label="Stale CRs (>5 days)"
              color="red"
              description="Open CRs with no reviewer activity for 5+ days"
              onClick={() => setShowStaleCrs(true)}
            />
          </div>
          ...
          >
            <button
              ...
              style={{
                ...
                fontSize: '14px',
                fontWeight: 600,
                background: pageView === 'visual' ? 'rgba(10,132,255,0.2)' : 'transparent',
                color: pageView === 'visual' ? '#60a5fa' : 'rgba(255,255,255,0.35)',
            ...
            <button
              ...
              style={{
                ...
                fontSize: '14px',
                fontWeight: 600,
                background: pageView === 'table' ? 'rgba(10,132,255,0.2)' : 'transparent',
                color: pageView === 'table' ? '#60a5fa' : 'rgba(255,255,255,0.35)',
                transition: 'all 0.25s',
              }}
            >
              📋 Table View
          ...
          {pageView === 'visual' && (
            <MetricsVisual
              ...
              onEngineerClick={(alias) => setSelectedEngineer(alias)}
            />
          )}
          ...
          {pageView === 'table' && (
            <>
              ...
              >
                👩‍💻 Engineers
                <span
                  style={{ fontSize: '11px', fontWeight: 400, color: 'rgba(255,255,255,0.25)' }}
                >
                  (click any row for 12-week drill-down)
                </span>
              </div>
                      ...
                      {[
                        ...
                        <th
                          ...
                          style={{
                            textAlign: 'left',
                            fontSize: '10px',
                            color: sortCol === h.key ? '#0a84ff' : 'rgba(255,255,255,0.35)',
                            textTransform: 'uppercase',
                            letterSpacing: '1px',
                            padding: '10px 14px',
                            borderBottom: '1px solid rgba(255,255,255,0.06)',
                  ...
                  <tbody>
                    {[...dashboard.engineers]
                      ...
                      .map((eng, idx) => {
                        ...
                        const rowBorder = isDeclining
                          ? '3px solid #ff9f0a'
                          : '3px solid transparent';
                        return (
                          <tr
                            key={eng.alias}
                            onClick={() => setSelectedEngineer(eng.alias)}
                            style={{
                              cursor: 'pointer',
                              transition: 'background 0.15s',
                              background: rowBg,
                            ...
                            onMouseEnter={(e) =>
                              (e.currentTarget.style.background = isDeclining
                          ...
                          >
                            ...
                            >
                              <span style={{ fontWeight: 600, color: CrColor(eng.crsCreated) }}>
                                {eng.crsCreated}
                              </span>
                            ...
                            >
                              <span
                                style={{ fontWeight: 600, color: ReviewColor(eng.crsReviewed) }}
                              >
                                {eng.crsReviewed}
                              </span>
        ...
        <>
          ...
          <div
            style={{
              ...
              background: 'rgba(12,12,20,0.98)',
              backdropFilter: 'blur(24px)',
              borderLeft: '1px solid rgba(255,69,58,0.2)',
              zIndex: 1000,
              overflowY: 'scroll',
              WebkitOverflowScrolling: 'touch',
              padding: '32px',
          ...
          >
            ...
            >
              ...
              >
                🔴 Stale CRs (
                {dashboard?.alerts?.staleCrDetails?.length || dashboard?.alerts?.staleCrs || 0})
              </h3>
              <button
                onClick={() => setShowStaleCrs(false)}
                style={{
            ...
            {dashboard?.alerts?.staleCrDetails && dashboard.alerts.staleCrDetails.length > 0 ? (
              dashboard.alerts.staleCrDetails.map((cr, i) => (
                <a
                  key={i}
                  href={`https://code.amazon.com/reviews/${cr.crId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{
                    display: 'block',
                ...
                >
                  ...
                  >
                    <span>
                      👤 Author:{' '}
                      <strong style={{ color: 'rgba(255,255,255,0.7)' }}>{cr.alias}</strong>
                    </span>
                ...
                {dashboard?.alerts?.staleCrs > 0 ? (
                  <>
                    ...
                    <div style={{ fontSize: '14px', fontWeight: 600 }}>
                      Stale CR details not yet loaded
                    </div>
                    <div style={{ fontSize: '12px', marginTop: '6px' }}>
                      Details are fetched in the background. Try refreshing the page.
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div style={{ fontSize: '12px', marginTop: '6px' }}>
                      All open CRs have reviewer activity within the last 5 days.
                    </div>
                  </>
                )}
              </div>
