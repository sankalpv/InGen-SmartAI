...
function init() {
  return new Promise((resolve, reject) => {
    ...
    db = new sqlite3.Database(DB_PATH, (err) => {
      if (err) {
        logger.error('Failed to open eng-metrics database:', err.message);
        return reject(err);
      }
      db.exec('PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON;', () => {
        db.exec(SCHEMA_SQL, (err) => {
          if (err) {
            logger.error('Failed to create eng-metrics schema:', err.message);
            return reject(err);
          }
...
async function fetchUrlsChunked(mcpClient, urls, chunkSize = BATCH_CHUNK_SIZE) {
  ...
  for (let i = 0; i < urls.length; i += chunkSize) {
    ...
    // Fetch sequentially to prevent hitting Amazon Midway / code.amazon.com rate limits (Max retry attempts)
    for (let j = 0; j < chunk.length; j++) {
      ...
      try {
        ...
        // Unpack standard MCP text content format
        let parsed = { text: '' };
        ...
        logger.error(`MCP fetch failed for url ${url}: ${err.message}`);
        chunkResults.push({ text: '' });
      }

      // Stagger 150ms between requests to prevent 500 Max Retry Errors
      if (j < chunk.length - 1) {
        await new Promise((r) => setTimeout(r, 150));
      }
    }

    allContent.push(...chunkResults);
    if (i + chunkSize < urls.length) await new Promise((r) => setTimeout(r, 400));
  }
  return allContent;
}
...
function unwrapMcpContentItem(item) {
  ...
  if (typeof raw === 'object' && raw !== null) {
    ...
    if (raw.content && typeof raw.content === 'object' && raw.content.content) {
      return typeof raw.content.content === 'string'
        ? raw.content.content
        : JSON.stringify(raw.content.content);
    }
    // Handle { content: "..." }
    if (raw.content && typeof raw.content === 'string') return raw.content;
    // Handle { status: "success", content: "..." }
    if (raw.status === 'success' && raw.content)
...
async function fetchOrgCodeActivityBatched(aliases, startDate, endDate) {
  ...
  try {
    ...
    const commitUrls = aliases.map(
      (a) =>
        `https://code.amazon.com/api/asci/changes_for_user?user=${a}&from_date=${startDate}&to_date=${endDate}`
    );
    logger.info(
      `Fetching commits for ${aliases.length} engineers in chunks of ${BATCH_CHUNK_SIZE}...`
    );
    ...
    for (let idx = 0; idx < commitsContent.length && idx < aliases.length; idx++) {
      ...
      try {
        ...
        /* skip parse error for this alias */
      }
    }
    ...
    const fromUrls = aliases.map(
      (a) =>
        `https://code.amazon.com/reviews/from-user/${a}?open=true&shipped=true&start_time=${startDate}&end_time=${endDate}`
    );
    logger.info(
      `Fetching CRs-created for ${aliases.length} engineers in chunks of ${BATCH_CHUNK_SIZE}...`
    );
    ...
    for (let idx = 0; idx < fromContent.length && idx < aliases.length; idx++) {
      ...
      const rawStr =
        typeof fromContent[idx] === 'string' ? fromContent[idx] : JSON.stringify(fromContent[idx]);
    ...
    const toUrls = aliases.map(
      (a) =>
        `https://code.amazon.com/reviews/to-user/${a}?open=true&shipped=true&start_time=${startDate}&end_time=${endDate}`
    );
    logger.info(
      `Fetching CRs-reviewed for ${aliases.length} engineers in chunks of ${BATCH_CHUNK_SIZE}...`
    );
    ...
    for (let idx = 0; idx < toContent.length && idx < aliases.length; idx++) {
      ...
      // Also keep raw stringified version for metadata extraction
      const rawStr =
        typeof toContent[idx] === 'string' ? toContent[idx] : JSON.stringify(toContent[idx]);
...
async function enrichCrDetailsFromCode(results) {
  ...
  for (let i = 0; i < crIds.length; i += CR_BATCH) {
    ...
    const urls = batch.map(
      (id) => `https://code.amazon.com/reviews/${id}?include-all-comments=true`
    );
      ...
      if (Array.isArray(contentItems)) {
        for (let idx = 0; idx < contentItems.length && idx < batch.length; idx++) {
          ...
          try {
            ...
            // Packages
            const packages = (crRevision.packages || [])
              .map((p) => p?.package?.name || p?.name)
              .filter(Boolean);
  ...
  logger.info(
    `CR enrichment complete: ${enrichedCount} CR entries enriched from ${Object.keys(enrichedMap).length} unique CRs`
  );
  return results;
}
...
function getYearWeekIds(year = new Date().getFullYear()) {
  ...
  // Include current (in-progress) week so real-time trends are visible in charts
...
async function backfillYear(year = new Date().getFullYear(), onProgress = null) {
  ...
  backfillState = {
    ...
    currentPhase: null,
    completedWeeks: 0,
    totalWeeks: missingWeeks.length,
    totalEngineers: aliases.length,
    weekStatuses: weekLabels,
    error: null,
    result: null,
  };
  ...
  logger.info(
    `Backfilling ${missingWeeks.length} weeks for ${aliases.length} engineers using BATCHED fetch (3 calls/week)`
  );
  ...
  for (let wi = 0; wi < missingWeeks.length; wi++) {
    ...
    logger.info(
      `Backfilling ${weekId} (${dateRange.start} to ${dateRange.end}) [${wi + 1}/${missingWeeks.length}]`
    );

    // Batched fetch — 3 MCP calls for ALL engineers
    const batchedResults = await fetchOrgCodeActivityBatched(
      aliases,
      dateRange.start,
      dateRange.end
    );
    ...
    if (onProgress)
      onProgress({
        ...
        engineers: aliases.length,
      });
  }

  // Update metadata
  await dbRun(`INSERT OR REPLACE INTO eng_metrics_meta (key, value) VALUES ('lastFetched', ?)`, [
    new Date().toISOString(),
  ]);
  await dbRun(`INSERT OR REPLACE INTO eng_metrics_meta (key, value) VALUES ('lastBackfill', ?)`, [
    new Date().toISOString(),
  ]);
  ...
  logger.info(
    `Backfill complete: ${totalStored} records stored across ${missingWeeks.length} weeks`
  );
  return result;
}
...
function startBackfillAsync(year = new Date().getFullYear()) {
  ...
  return {
    status: 'started',
    message:
      'Backfill started in background. Poll /api/eng-metrics?view=backfill-status for progress.',
  };
}
...
async function incrementalSync() {
  ...
  if (prevWeekRow) {
    ...
    if (fetchedDate < prevWeekEnd) {
      ...
      logger.info(
        `Incremental sync: re-fetching ${prevWeekId} (was fetched mid-week on ${prevWeekRow.fetched_at})`
      );
...
async function countOrgStaleCrs() {
  ...
  try {
    ...
    const openUrls = aliases.map((a) => `https://code.amazon.com/reviews/from-user/${a}?open=true`);
    logger.info(
      `Counting stale CRs for ${aliases.length} engineers in chunks of ${BATCH_CHUNK_SIZE}...`
    );
...
async function fetchOrgMetrics(weekId = null) {
  ...
  for (const member of allMembers) {
    try {
      logger.info(
        `Fetching metrics for ${member.alias} (${fetchedCount + 1}/${allMembers.length})`
      );
  ...
  await dbRun(`INSERT OR REPLACE INTO eng_metrics_meta (key, value) VALUES ('lastWeekId', ?)`, [
    currentWeekId,
  ]);
...
async function getOrgDashboard(weekId = null) {
  ...
  // Current week metrics (unfiltered)
  const rawEngineers = await dbAll(
    `SELECT * FROM eng_metrics_weekly WHERE week_id = ? ORDER BY crs_created DESC`,
    [currentWeekId]
  );

  // Filter to only include engineers officially in the local org-store
  const orgStore = require('./org-store');
  const { members } = await orgStore.getRelevantSDEs();
  const validAliases = new Set(members.map((m) => m.alias));
  const engineers = rawEngineers.filter((e) => validAliases.has(e.alias));
  ...
  const turnarounds = engineers
    .map((e) => e.avg_turnaround_hours)
    .filter((t) => t > 0)
    .sort((a, b) => a - b);
  const p50Turnaround =
    turnarounds.length > 0 ? turnarounds[Math.floor(turnarounds.length / 2)] : 0;
  ...
  const prevTurnarounds = prevEngineers
    .map((e) => e.avg_turnaround_hours)
    .filter((t) => t > 0)
    .sort((a, b) => a - b);
  const prevP50 =
    prevTurnarounds.length > 0 ? prevTurnarounds[Math.floor(prevTurnarounds.length / 2)] : 0;

  const pctChange = (cur, prev) => (prev > 0 ? Math.round(((cur - prev) / prev) * 100) : 0);

  // Bus factor risks
  const busFactorRisks = await dbAll(`SELECT * FROM eng_bus_factor WHERE week_id = ?`, [
    currentWeekId,
  ]);

  // Get last fetched time
  const lastFetchedRow = await dbGet(
    `SELECT value FROM eng_metrics_meta WHERE key = 'lastFetched'`
  );
  ...
  return {
    ...
    summary: {
      ...
      p50Turnaround: {
        value: p50Turnaround,
        display: `${p50Turnaround.toFixed(1)}h`,
        prevDisplay: `${prevP50.toFixed(1)}h`,
      },
    ...
    engineers: engineers.map((e) => {
      ...
      const reviewRatio =
        e.crs_created > 0 ? Math.round((e.crs_reviewed / e.crs_created) * 10) / 10 : null;
      ...
      const prevRR =
        w1 && w1.crs_created > 0 ? Math.round((w1.crs_reviewed / w1.crs_created) * 10) / 10 : null;
      ...
      const reviewRatioDelta =
        reviewRatio !== null && prevRR !== null
      ...
      if (w3 && w2 && w1) {
        if (
          w3.crs_created > w2.crs_created &&
          w2.crs_created > w1.crs_created &&
          w1.crs_created > e.crs_created
        ) {
          decliningMetrics.push('crsCreated');
        }
        if (
          w3.crs_reviewed > w2.crs_reviewed &&
          w2.crs_reviewed > w1.crs_reviewed &&
          w1.crs_reviewed > e.crs_reviewed
        ) {
...
async function getEngineerDetail(alias, weeks = 12) {
  ...
  return {
    ...
    weeklyHistory: history.map((h) => {
      let crDetails = [];
      try {
        crDetails = JSON.parse(h.cr_details_json || '[]');
      } catch (e) {
        /* ignore */
      }
      return {
    ...
    if (wbrData && wbrData.sections) {
      ...
      for (const goal of allGoals) {
        ...
        for (const eng of engineers) {
          let crDetails = [];
          try {
            crDetails = JSON.parse(eng.cr_details_json || '[]');
          } catch (e) {
            /* ignore */
          }
...
function close() {
  if (db) {
    db.close();
    db = null;
    logger.info('Eng-metrics database closed');
  }
}

// ─── View-As Scoped Functions ───

/**
 * Get dashboard data scoped to a specific set of aliases, grouped by manager.
 * @param {Array} aliases - Flat list of engineer aliases
 * @param {Object} managerMap - Map of alias → { managerAlias, managerName, l7Alias, l7Name }
 * @param {string?} weekId - Optional week ID
 */
async function getOrgDashboardForAliases(aliases, managerMap = {}, weekId = null) {
  await init();
  ...
  // Fetch current week data for our alias list
  const placeholders = aliases.map(() => '?').join(',');
  const engineers = await dbAll(
    `SELECT * FROM eng_metrics_weekly WHERE week_id = ? AND alias IN (${placeholders}) ORDER BY crs_created DESC`,
    [currentWeekId, ...aliases]
  );

  // Fetch previous week for trend
  const prevEngineers = await dbAll(
    `SELECT * FROM eng_metrics_weekly WHERE week_id = ? AND alias IN (${placeholders})`,
    [prevWeekId, ...aliases]
  );
  ...
  // Map engineers + group by manager
  const engineerList = engineers.map((e) => {
    const mgrInfo = managerMap[e.alias] || {};
    const reviewRatio =
      e.crs_created > 0 ? Math.round((e.crs_reviewed / e.crs_created) * 10) / 10 : null;
    return {
      alias: e.alias,
      ...
      linesChanged: e.lines_changed,
      linesDisplay: formatLines(e.lines_changed),
      managerAlias: mgrInfo.managerAlias || e.team || '',
      managerName: mgrInfo.managerName || '',
      l7Alias: mgrInfo.l7Alias || '',
      l7Name: mgrInfo.l7Name || '',
    };
  });
  ...
  return {
    ...
    summary: {
      crsCreated: { value: totalCrsCreated, trend: pctChange(totalCrsCreated, prevCrsCreated) },
      crsReviewed: { value: totalCrsReviewed, trend: pctChange(totalCrsReviewed, prevCrsReviewed) },
      linesChanged: { value: totalLinesChanged, display: formatLines(totalLinesChanged) },
    },
    engineers: engineerList,
    totalEngineers: engineers.length,
    hasData: engineers.length > 0,
  };
}

/**
 * Fetch + store metrics for a specific set of aliases for a given week.
 * Used by the View As backfill feature.
 * @param {Array} aliases - Flat list of engineer aliases
 * @param {Object} memberMap - Map of alias → { name, team, managerAlias }
 * @param {string?} weekId - Optional week ID
 */
async function fetchMetricsForAliases(aliases, memberMap = {}, weekId = null) {
  ...
  logger.info(`Fetching metrics for ${aliases.length} aliases (View As), week ${currentWeekId}`);
  ...
  try {
    ...
    logger.warn(`CR enrichment failed for View As ${currentWeekId}: ${e.message}`);
  }
  ...
  logger.info(`View As fetch complete: ${stored} engineer records stored for ${currentWeekId}`);
  return { weekId: currentWeekId, dateRange, fetchedCount: stored };
}

/**
 * Backfill a full year of data for a specific set of aliases.
 * Reuses the backfill state for progress tracking.
 * @param {Array} aliases - Flat list of engineer aliases
 * @param {Object} memberMap - Map of alias → { name, team, managerAlias }
 * @param {number} year - Year to backfill
 */
async function backfillForAliases(aliases, memberMap = {}, year = new Date().getFullYear()) {
  await init();

  // Figure out which weeks are missing for these aliases
  const allWeekIds = getYearWeekIds(year);
  // A week is "covered" if at least half the aliases have data
  const coveredWeeks = new Set();
  for (const wid of allWeekIds) {
    const placeholders = aliases.map(() => '?').join(',');
    const row = await dbGet(
      `SELECT COUNT(DISTINCT alias) as c FROM eng_metrics_weekly WHERE week_id = ? AND alias IN (${placeholders})`,
      [wid, ...aliases]
    );
    if ((row?.c || 0) >= Math.floor(aliases.length * 0.5)) {
      coveredWeeks.add(wid);
    }
  }
  const missingWeeks = allWeekIds.filter((wid) => !coveredWeeks.has(wid));
  ...
  logger.info(`View As backfill: ${missingWeeks.length} weeks for ${aliases.length} engineers`);
  ...
  for (let wi = 0; wi < missingWeeks.length; wi++) {
    ...
    await fetchMetricsForAliases(aliases, memberMap, weekId);
  ...
  const result = {
    status: 'complete',
    weeksProcessed: missingWeeks.length,
    totalFetches: missingWeeks.length * aliases.length,
  };
  backfillState.running = false;
  backfillState.result = result;
  backfillState.currentPhase = 'done';
  ...
  logger.info(
    `View As backfill complete: ${result.totalFetches} records across ${missingWeeks.length} weeks`
  );
  return result;
}
