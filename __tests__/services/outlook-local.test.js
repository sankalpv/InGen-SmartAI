...
jest.resetModules();
mod = require('../../services/outlook-local');
    });
    ...
    describe('clearCalendarCache', () => {
        it('should be defined', () => {
            expect(mod.clearCalendarCache || mod.default?.clearCalendarCache).toBeDefined();
        });

        it('should be accessible from module', () => {
            const val = mod.clearCalendarCache || mod.default?.clearCalendarCache;
            if (typeof val === 'function') {
                expect(typeof val).toBe('function');
            } else {
                expect(val).toBeDefined();
